...
export class MouseleaveBinderDirective extends InteractoBinderDirective<HTMLElement> {
   ...
   */
  @Input()
  set ioMouseleave(fn: (partialBinder: PartialPointBinder) => void)  {
    ...
    if (this.onDyn) {
      this.getComponent(fnName)[fnName](this.bindings.mouseLeaveBinder(withbubbling).onDynamic(this.element));
    }else {
      this.getComponent(fnName)[fnName](this.bindings.mouseLeaveBinder(withbubbling).on(this.element));
    }
  }
}
